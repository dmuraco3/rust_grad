...
impl<const VOCAB_SIZE: usize, const EMBED_SIZE: usize, E, D> Embedding<VOCAB_SIZE, EMBED_SIZE, E, D>
where
    E: Unit,
...
impl<const IN_FEATURES: usize, const OUT_FEATURES: usize, E, D>
    Linear<IN_FEATURES, OUT_FEATURES, E, D>
where
    E: Unit,
    D: Storage<E> + ZerosTensor<E>,
{
...
impl<const DIM: usize, E, D> RMSNorm<DIM, E, D>
where
    E: Unit,
    D: Storage<E> + ZerosTensor<E>,
{
...
impl<const DIM: usize, const HIDDEN_DIM: usize, E, D> FeedForward<DIM, HIDDEN_DIM, E, D>
where
    E: Unit,
    D: Storage<E> + ZerosTensor<E>,
{
...
impl<const DIM: usize, const HEADS: usize, const KVHEADS: usize, const MAXCONTENT: usize, E, D>
    Attention<DIM, HEADS, KVHEADS, MAXCONTENT, E, D>
where
    E: Unit,
    D: Storage<E> + ZerosTensor<E>,
{
...
impl<
        ...
        E,
        D,
    > TransformerBlock<DIM, HIDDEN_DIM, HEADS, KVHEADS, MAXCONTENT, E, D>
where
    E: Unit,
    D: Storage<E> + ZerosTensor<E>,
{
...
pub fn generate_rotary_matrix<E: Unit, D: Storage<E> + ZerosTensor<E>>(
    dim: usize,
    _end: usize,
    _theta: E,
    device: &D,
) -> Tensor<(Const<2>, usize), E, D> {
    let rotary_i = device.try_zeros_from(&(dim,)).unwrap();
    {
    ...
    let rotary_matrix = device.try_zeros_from(&(Const::<2>, dim)).unwrap();
...
impl<
        ...
        E,
        D,
    > Transformer<LAYERS, DIM, HIDDEN_DIM, HEADS, KVHEADS, MAXCONTENT, VOCAB_SIZE, E, D>
where
    E: Unit,
    D: Storage<E> + ZerosTensor<E>,
{
    pub fn init_with_device(device: &D) -> Self {
        Self {
            ...
            rotary_matrix: generate_rotary_matrix(
                DIM / HEADS,
                MAXCONTENT * 2,
                E::from_u32(10_000),
                device,
