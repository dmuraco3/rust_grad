...
use crate::{devices::cpu::CPU, dtypes::Unit, shape::Dim};
...
impl<E: Unit> MaxPool2DKernel<E> for CPU {
    ...
    >(
        ...
        let (src_inner, src_shape) = if (
            src.shape.0.size() % filter_shape.0.size(),
            src.shape.1.size() % filter_shape.1.size(),
        ) != (0, 0)
        {
            match padding {
                PADDING::VALID => {
                    ...
                    for el in src_inner
                        .index(0..drop_src_shape.0 * src.shape.1.size())
                        .chunks(src.shape.1.size())
                    {
                ...
                PADDING::SAME => {
                    ...
                    for el in src_inner
                        .index(0..src.shape.0.size() * src.shape.1.size())
                        .chunks(src.shape.1.size())
                    {
            ...
            (
                src.data.read().unwrap().clone(),
                (src.shape.0.size(), src.shape.1.size()),
            )
        };
            ...
            for out_x in 0..out_shape.1.size() {
                ...
                for filter_y in 0..filter_shape.0.size() {
                    for filter_x in 0..filter_shape.1.size() {
                        let l = src_inner.index(
                            (src_shape.1.size() * (out_y * stride.size() + filter_y))
                                + (out_x * stride.size() + filter_x),
                        );
