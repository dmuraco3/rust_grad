...
use crate::{
    ...
    shape::{Dim, Shape},
    storage::Storage,
    tensor::{tape::Gradients, Tensor, ZerosTensor},
};
    ...
    Self: MatMatImpl<E>,
{
    ...
    fn backward<I: Dim, J: Dim, K: Dim>(
        ...
        grads.try_alloc_for((
            &lhs.device,
            lhs.id,
            lhs.device.num_el(lhs.data.read().unwrap().to_owned()),
        ))?;
        grads.try_alloc_for((
            &rhs.device,
            rhs.id,
            rhs.device.num_el(rhs.data.read().unwrap().to_owned()),
        ))?;
        grads.try_ones_for((
            &out.device,
            out.id,
            out.device.num_el(out.data.read().unwrap().to_owned()),
        ))?;
