use crate::{
    ...
    dtypes::FloatUnit,
    shape::Shape,
    tensor::{tape::Gradients, Tensor},
};

use super::SCCEKernel;

impl<E: FloatUnit> SCCEKernel<E> for CPU {
    fn forward<S: Shape, OutShape: Shape>(
        src: &Tensor<S, E, Self>,
        ...
        let loss = src
            ...
            .unwrap()
            .iter()
            .zip(actual.data.read().unwrap().iter())
            .fold(E::ZERO, |acc, (&p, &t): (&E, &E)| acc + t * p.log_10());
    ...
    fn backward<S: Shape>(
        ...
        src_grad
            ...
            .for_each(|((grad, src), actual)| *grad = *src - *actual);
